...
function DeleteTask(buttonElement) {
	...
	const task = buttonElement.parentElement.parentElement; // retrieve the task's <li> element
	task.remove();
...
function MoveTask(buttonElement, direction) {
	...
	const listContainer = taskElement.parentElement;

	if (direction === "up") {
		const taskAbove = taskElement.previousElementSibling;
		if (taskAbove) {
			listContainer.insertBefore(taskElement, taskAbove);
		}
	}
	else if (direction === "down") {
		const taskBelow = taskElement.nextElementSibling;
		if (taskBelow){
			listContainer.insertBefore(taskElement, taskBelow.nextElementSibling);
		}
	}
...
function MoveList(buttonElement, direction) {
	...
	const listElement = buttonElement.parentElement.parentElement;
	const boardContainer = listElement.parentElement;

	if (direction === "left") {
		const listAbove = listElement.previousElementSibling;
		if (listAbove) {
			boardContainer.insertBefore(listElement, listAbove);
		}
	}
	else if (direction === "right") {
		const listBelow = listElement.nextElementSibling;
		if (listBelow) {
			boardContainer.insertBefore(listElement, listBelow.nextElementSibling);
		}
	}
...
function MoveBoard(boardId, direction, buttonElement) {
	console.log("attempted to move board: " + boardId)

	const boardElement = buttonElement.parentElement.parentElement;
	const sidebarContainer = boardElement.parentElement;

	if (direction === "up") {
		const boardAbove = boardElement.previousElementSibling;
		if (boardAbove) {
			sidebarContainer.insertBefore(boardElement, boardAbove);
		}
	}
	else if (direction === "down") {
		const boardBelow = boardElement.nextElementSibling;
		if (boardBelow) {
			sidebarContainer.insertBefore(boardElement, boardBelow.nextElementSibling);
		}
	}
...
function OpenBoardOptions(buttonElement, boardId, boardName, boardDesc) {
	...
	const displayBoardMembers = () => {
		let membersPanelHTML = `
		<h1>Board members</h1>
			<ul><li><b>Admin: </b>nick</li>
				<li>
					Michael
					<button class="remove-member" onclick="RemoveMember('Michael', 6, this)">✕</button>
				</li>
				<li>
					amy123
					<button class="remove-member" onclick="RemoveMember('amy123', 6, this)">✕</button>
				</li>
			</ul>`;
        sidePanel.innerHTML = membersPanelHTML;
...
function RemoveMember(memberName, boardId, buttonElement) {
    ...
    buttonElement.parentElement.remove(); // remove the member from the list in the panel
